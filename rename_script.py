...
if __name__ == "__main__":
    ...
    # 指定目标文件夹
    folder_path = r"C:\Users\zhangbon\Desktop\数据产权\知识产权整理稿\收入审核预测-更改版\总\省份预测结果"
    for filename in os.listdir(folder_path):
        new_name = filename.replace("_预测金额结果","门店销售金额预测数据")
        new_file = os.path.join(folder_path, new_name)
        os.rename(os.path.join(folder_path, filename), new_file)
