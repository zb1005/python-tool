...
def batch_rename_columns():
    ...
    # 定义文本替换规则
    text_replacements = {
        "本月的预测销售金额": "本月预测销售金额(万元)",
        "前推6个月审核金额": "预测月前6个月审核金额(万元)",
        "前推5个月审核金额": "预测月前5个月审核金额(万元)",
        "前推4个月审核金额": "预测月前4个月审核金额(万元)",
        "前推3个月审核金额": "预测月前3个月审核金额(万元)",
        "前推2个月审核金额": "预测月前2个月审核金额(万元)",
        "前推1个月审核金额": "预测月前1个月审核金额(万元)"
    }
                ...
                for col in df.columns:
                    if df[col].dtype == 'object':  # 仅处理字符串列
                        for old_text, new_text in text_replacements.items():
                            if df[col].str.contains(old_text).any():
                                df[col] = df[col].str.replace(old_text, new_text, regex=False)
                                text_modified = True
                                print(f"    替换文本：'{old_text}' -> '{new_text}' 在列 '{col}'")
                
                if modified or text_modified:
                    # 保存修改后的文件（覆盖原文件）
                    df.to_excel(file_path, index=False, engine='openpyxl')
                    processed_files.append(file_path)
                    print(f"  ✓ 已处理：{os.path.basename(file_path)}")
                else:
                    print(f"  - 跳过：{os.path.basename(file_path)}（未找到需要修改的列或文本）")
...
if __name__ == "__main__":
    print("开始批量重命名Excel列名和文本替换...")
    print("规则1：将2024年7-12月审核金额改为预测月前6-1个月审核金额")
    print("规则2：文本替换：'本月的预测销售金额'->'本月预测销售金额'，'前推n个月审核金额'->'预测月前n个月审核金额(万元)'")
