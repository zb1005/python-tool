...
def process_approval_data():
    # 读取四个Excel文件
    base_df = pd.read_excel(r"E:\000000我的事项\2025-06\1131统计\Q2审批统计\审批结果(加了延期时长为0的逻辑)-0703-新.xlsx", engine='openpyxl')
    holiday_df = pd.read_excel(r"E:\000000我的事项\2025-06\1131统计\Q2审批统计\2025非工作日清单-截至Q2.xlsx", engine='openpyxl')
    special_node_df = pd.read_excel(r"E:\000000我的事项\2025-06\1131统计\Q2审批统计\特殊节点时长基准-250618.xlsx", engine='openpyxl')
    ...
    base_df['延期时长情况：0；≤1；1<X≤2；2<X≤3；>3'] = base_df['节点审批延期时长(实际工作时长-节点审批时长）'].apply(
        lambda x: '0' if x == 0 else ('≤1' if x <= 1 else ('1<X≤2' if 1 < x <= 2 else ('2<X≤3' if 2 < x <= 3 else '>3')))
    )
...
if __name__ == '__main__':
    ...
    merged_data.to_excel(r'E:\000000我的事项\2025-06\1131统计\Q2审批统计\审批结果.xlsx', index=False)
