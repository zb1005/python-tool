...
# 导入数据筛选模块
try:
    from data_filter import filter_data_for_comparison
    FILTER_MODULE_AVAILABLE = True
except ImportError:
    FILTER_MODULE_AVAILABLE = False
    print("⚠️ 数据筛选模块未找到，将跳过筛选功能")
...
class DataComparisonApp:
    ...
    def setup_ui(self):
        ...
        # 数据筛选选项
        self.enable_filter = tk.BooleanVar(value=True)
        ttk.Checkbutton(config_frame, text="启用数据筛选功能", variable=self.enable_filter).grid(row=3, column=0, columnspan=3, sticky=tk.W, pady=5)
        
        # 源文件列名显示区域
        ttk.Label(config_frame, text="源文件列名:").grid(row=4, column=0, sticky=tk.W, pady=5)
        self.source_columns_text = tk.Text(config_frame, height=3, width=50, wrap=tk.WORD)
        self.source_columns_text.grid(row=4, column=1, sticky=(tk.W, tk.E), padx=5)
        ...
        source_columns_scrollbar.grid(row=4, column=2, sticky=(tk.N, tk.S))
        
        # 目标文件列名显示区域
        ttk.Label(config_frame, text="目标文件列名:").grid(row=5, column=0, sticky=tk.W, pady=5)
        self.target_columns_text = tk.Text(config_frame, height=3, width=50, wrap=tk.WORD)
        self.target_columns_text.grid(row=5, column=1, sticky=(tk.W, tk.E), padx=5)
        ...
        target_columns_scrollbar.grid(row=5, column=2, sticky=(tk.N, tk.S))
        
        # 输出目录
        ttk.Label(config_frame, text="输出目录:").grid(row=6, column=0, sticky=tk.W, pady=5)
        ttk.Entry(config_frame, textvariable=self.output_dir, width=50).grid(row=6, column=1, sticky=(tk.W, tk.E), padx=5)
        ttk.Button(config_frame, text="浏览", command=self.select_output_dir).grid(row=6, column=2, padx=5)
    ...
    def select_source_file(self):
        ...
        if filename:
            ...
            self.source_path.set(abs_filename)
            self.log_message(f"已选择源文件: {os.path.basename(abs_filename)}")
            self.log_message(f"源文件路径: {abs_filename}")
            
            # 显示源文件列名
            columns = self.get_file_columns(abs_filename)
            if columns:
    ...
    def add_target_file(self):
        ...
        if filenames:
            ...
            for i, filename in enumerate(filenames, 1):
                # 确保路径是绝对路径
                abs_filename = os.path.abspath(filename)
                if abs_filename not in self.target_paths:
                    self.target_paths.append(abs_filename)
                    self.target_listbox.insert(tk.END, os.path.basename(abs_filename))
                    self.log_message(f"已添加目标文件: {os.path.basename(abs_filename)}")
                    self.log_message(f"目标文件路径: {abs_filename}")
    ...
    def clear_all(self):
        ...
        self.enable_filter.set(True)  # 重置筛选选项为启用
        ...
        try:
            ...
            if self.enable_filter.get():
                if FILTER_MODULE_AVAILABLE:
                    self.log_message("✅ 数据筛选功能已启用")
                else:
                    self.log_message("⚠️ 筛选功能已启用但筛选模块不可用，将跳过筛选")
            else:
                self.log_message("ℹ️ 筛选功能已禁用，将使用完整源数据进行比对")
            
            # 运行简化版的比对
            filter_results = batch_compare_simple(source_path, self.target_paths, key_columns, output_dir, self.enable_filter.get())
            
            # 显示筛选结果
            if filter_results:
                self.log_message("\n📊 筛选结果汇总:")
                for target_name, result_info in filter_results.items():
                    if result_info['filtered']:
                        self.log_message(f"   • {target_name}: {result_info['original_count']} → {result_info['filtered_count']} 条记录 (筛选后)")
                    else:
                        self.log_message(f"   • {target_name}: {result_info['original_count']} 条记录 (使用原始数据)")
...
def batch_compare_simple(source_path, target_paths, key_columns, output_dir, enable_filter=True):
    """简化的批量比对主程序"""
    ...
    try:
        ...
        print(f"源文件读取完成，记录数: {original_count:,}")
        
        # 数据筛选功能（如果可用且启用）
        filtered_source_data = {}
        if enable_filter and FILTER_MODULE_AVAILABLE and target_paths:
            print("\n🔍 开始数据筛选...")
            filtered_source_data = filter_data_for_comparison(source_df, target_paths, key_columns)
            print("✅ 数据筛选完成")
        elif enable_filter and not FILTER_MODULE_AVAILABLE:
            print("⚠️ 筛选功能已启用但筛选模块不可用，跳过筛选")
        else:
            print("ℹ️ 筛选功能已禁用，跳过数据筛选")
        ...
        for i, target_path in enumerate(target_paths, 1):
            ...
            try:
                ...
                # 确定要使用的源数据（筛选后的或原始的）
                if target_name in filtered_source_data and not filtered_source_data[target_name].empty:
                    current_source_df = filtered_source_data[target_name]
                    filtered_count = len(current_source_df)
                    print(f"📊 使用筛选后的源数据: {filtered_count} 条记录")
                    filter_results[target_name] = {
                        'filtered': True,
                        'original_count': original_count,
                        'filtered_count': filtered_count
                    }
                else:
                    current_source_df = source_df.copy()  # 使用copy确保每次核对都是独立的
                    print("📊 使用原始源数据")
                    filter_results[target_name] = {
                        'filtered': False,
                        'original_count': original_count,
                ...
                # 比对数据
                result_df = compare_datasets(current_source_df, target_df, original_columns)
