...
if __name__ == "__main__":
    input_file = r"C:\Users\zhangbon\Desktop\1.XLSX"  # 原始Excel路径
    output_file = r"C:\Users\zhangbon\Desktop\1_计算时长.xlsx"  # 输出路径
    df = pd.read_excel(input_file)
    # 转换时间列为datetime类型
    df["工作组首次响应时间"] = pd.to_datetime(df["工作组首次响应时间"], errors='coerce')
    df["诉求创建时间"] = pd.to_datetime(df["诉求创建时间"], errors='coerce')
    df["时长"] = df.apply(lambda row: calculate_work_hours(row["诉求创建时间"], row["工作组首次响应时间"]), axis=1)
